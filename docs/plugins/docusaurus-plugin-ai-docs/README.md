...
Markdown files are generated during Docusaurus' production build, via this plugin's `postBuild` hook. They are not generated by the development preview command (`npm run docs:preview`), because that runs `docusaurus start`.

From the repo root, build and serve the production docs:
...
npm run serve -w docs -- --no-open --port 4444
