...
test('GET only returns a different etag if the resource changed', async () => {
  ...
  // Create a resource
  const postResponse = await sendRequest(tembaServer, 'post', '/stuff', { name: 'item 1' })
  expectSuccess(postResponse)
  const id = postResponse.body.id

  // Get the created resource
  const getResponse = await sendRequest(tembaServer, 'get', '/stuff/' + id)
  expectSuccess(getResponse)
  const etag1 = getResponse.headers['etag']

  // Get the created resource again
  const getResponse2 = await sendRequest(tembaServer, 'get', '/stuff/' + id)
  ...
  const getResponse3 = await sendRequest(tembaServer, 'get', '/stuff/' + id)
...
test('GET with If-None-Match returns 304 Not Modified if etag is the same', async () => {
  ...
  const getResponse3 = await sendRequest(tembaServer, 'get', '/stuff/' + id, null, {
    'If-None-Match': etag,
  })
  expectSuccess(getResponse3)
...
test('PUT requires an If-Match header with an up to date etag', async () => {
  ...
  const putResponse = await sendRequest(tembaServer, 'put', '/stuff/' + id, { name: 'item 2' })
  expect(putResponse.statusCode).toEqual(412)

  // Now try to update the resource with the etag, which should work
  const putResponse2 = await sendRequest(
    tembaServer,
    'put',
    '/stuff/' + id,
    {
  ...
  const putResponse3 = await sendRequest(
    tembaServer,
    'put',
    '/stuff/' + id,
    {
...
test('PATCH requires an If-Match header with an up to date etag', async () => {
  ...
  // Try to update the resource without an If-Match header, which is not allowed
  const patchResponse = await sendRequest(tembaServer, 'patch', '/stuff/' + id, { name: 'item 2' })
  expect(patchResponse.statusCode).toEqual(412)

  // Now try to update the resource with the etag, which should work
  const patchResponse2 = await sendRequest(
    tembaServer,
    'patch',
    '/stuff/' + id,
    {
  ...
  // Try to update the resource again, but now with the old etag, which is also not allowed
  const patchResponse3 = await sendRequest(
    tembaServer,
    'patch',
    '/stuff/' + id,
    {
...
test.only('DELETE an item requires an If-Match header with an up to date etag', async () => {
  const tembaServer = createServer({ etags: true } satisfies UserConfig)
  ...
  const deleteResponse = await sendRequest(tembaServer, 'delete', '/stuff/' + id)
  expect(deleteResponse.statusCode).toEqual(412)

  // Now try to delete the resource with the etag, which should work
  const deleteResponse2 = await sendRequest(tembaServer, 'delete', '/stuff/' + id, null, {
    'If-Match': etag,
  })
  expectSuccess(deleteResponse2)

  // Try to delete the resource again, which is fine because it's already gone
  const deleteResponse3 = await sendRequest(tembaServer, 'delete', '/stuff/' + id, null, {
    'If-Match': etag,
  })
